...
if (getUserRole() !== 'owner') {
    ...
    redirect('/apps/karyawan/dashboard.php');
}
...
try {
    ...
    if (!$ownerData['bisnis_id'] && $currentPage !== 'setup_bisnis.php') {
        if (isset($_SERVER['HTTP_X_REQUESTED_WITH']) && $_SERVER['HTTP_X_REQUESTED_WITH'] === 'XMLHttpRequest') {
            sendResponse(false, 'Business setup required', ['redirect' => '/apps/owner/setup_bisnis.php']);
            exit;
        }
        redirect('/apps/owner/setup_bisnis.php');
    }
